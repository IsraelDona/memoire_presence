...
function Login() {
  ...
  const [showPassword, setShowPassword] = useState(false); // État conservé
  ...
  const needsFaceEnrollment = Boolean(
    ...
    (normalizeRole(user?.role) === 'AGENT' ||
      normalizeRole(user?.role) === 'CHEF_SERVICE') &&
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (result?.kind === 'authenticated' && result.session) {
        if ((normalizeRole(result.session.role) === 'AGENT' ||
          normalizeRole(result.session.role) === 'CHEF_SERVICE') &&
          result.session.visageEnregistre === false) {
