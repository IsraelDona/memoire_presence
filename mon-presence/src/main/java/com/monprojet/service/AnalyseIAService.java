    ...
    public AnalyseIA genererAnalyse(Long utilisateurId) {
        ...
        long joursCollectes =
                ChronoUnit.DAYS.between(
                        utilisateur.getDateDebutCycleAnalyse(), aujourdHui) + 1;

        boolean cycleComplet = joursCollectes >= 14;

        long presences =
                presenceRepository.countByUtilisateurAndDatePresenceGreaterThanEqual(
                        utilisateur, utilisateur.getDateDebutCycleAnalyse());
        ...
        double tauxPresence = (presences * 100.0) / (double) joursCollectes;
