...
public class ChefServiceController {
    ...
    public List<Presence> getPresencesAgents(Authentication authentication) {
        ...
        return presenceRepository.findByUtilisateurServiceIdOrderByDatePresenceDescHeurePointageDesc(
    ...
    public List<Utilisateur> getAgentsDuService(Authentication authentication) {
        ...
        // Exclure le chef connecté de la liste de ses agents
        return utilisateurRepository
                .findByServiceId(chef.getService().getId())
                .stream()
                .filter(u -> !u.getEmail().equals(chef.getEmail()))
                .collect(java.util.stream.Collectors.toList());
    }
    /*
     * Justificatifs — uniquement les agents du service du chef connecté
     */
    @GetMapping("/justificatifs")
    public List<Justificatif> getJustificatifs(Authentication authentication) {
        Utilisateur chef = getChefConnecte(authentication);
        ...
        return justificatifRepository
                .findByUtilisateurServiceId(chef.getService().getId());
    }
    ...
    public String supprimerMission(@PathVariable Long missionId) {
        return missionService.supprimerMission(missionId);
    }

    @DeleteMapping("/reunions/{reunionId}")
    public String supprimerReunion(@PathVariable Long reunionId) {
        return reunionService.supprimerReunion(reunionId);
    }
}