...
public class PresenceController {
    ...
    public ResponseEntity<String> verifierVisage(
            @RequestBody com.monprojet.dto.FaceRegistrationRequest request) {

        boolean estReconnu = presenceService.verifierVisage(request);

        if (estReconnu) {
            return ResponseEntity.ok("Visage reconnu avec succès");
        }

        return ResponseEntity
                .status(HttpStatus.UNAUTHORIZED)
                .body("Échec de la reconnaissance faciale : visage non reconnu ou introuvable");
    }
}